          ...
          <div class="x_panel">
            <div class="x_title">
              ...
              <ul class="nav navbar-right panel_toolbox">
                <li class="dropdown">
                  <a href="{{route('category.add')}}" class="label label-primary" style="color:white"><i class="fa fa-plus"></i>  Add  Category</a>
                </li>
              </ul>
              <div class="clearfix"></div>
            </div>
              ...
              <table id="dstable" class="table table-striped table-bordered" cellpadding="0">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="btnrow"><a href="category/edit/{{$row->id}}" class="btn-edit label label-info"><i class="fa fa-edit"></i></a>
                     <a data-url="category/delete/{{$row->id}}" href="javascript:void(0)" class="btn-delete label label-danger deletecat"><i class="fa fa-trash"></i></a></td>
   ...
   <script type="text/javascript">
    $(document).ready(function(){
     ...
     $(document).on('click','.deletecat',function(){
      ...
      Swal.fire({
        title: "Are you sure?",
        text: "Once delete it, you can not recover this category!",
        type: "warning",
        ...
        if(willdelete.value){
          $.ajax({
            ...
            {
              window.location  = "{{ route('category.display') }}?msg=3";
            }
          });
        }
        else
     ...
     $('body').on('change','.js-switch',function(e){
      ...
      if($(this).is(':checked'))
      {
       text1 = "Once you change the status,this category will available";

     }
     else
     {
      text1 = "Once you change the status,this category no longer available";
    }
    Swal.fire({
      title: "Are you sure?",
      text: text1,
      ...
      if(willdelete.value){
        $.ajax({
          url:"{{route('category.activeOrDis')}}",
          type:"POST",
          data:{
            "_token": "{{ csrf_token() }}",
            "checkbox":check,
      ...
      {
        window.location  = "{{ route('category.display') }}";
      }
    })
